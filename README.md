...
{ "name": "gr55_set_string_level", "arguments": { "string": 1, "source": "pcm1", "value": 80, "mode": "staged" } }
{ "name": "gr55_stage_control_mapping", "arguments": { "controlId": "ctlPedal", "targetId": "delayLevel" } }
{ "name": "gr55_send_staged", "arguments": {} }
{ "name": "gr55_save_with_readback", "arguments": { "bank": 73, "slot": 3, "safety": true } }
{ "name": "gr55_export_mapped_patch", "arguments": { "format": "syx" } }
```

The MCP server also exposes string matrix tools, physical-control/assign-target listing, assign staging, import preview and safety-report tools. Destructive MCP paths require an explicit safety flag, a selected USER slot and a recorded mapped backup. `gr55_backup_user_73_3` is a mapped temporary-patch backup workflow, not a full Roland bulk dump.
...
- PCM1, PCM2, Modeling/COSM and Normal PU source controls wired through the parameter registry. USER 73-3 read verification passed for the mapped source set; individual controls still marked `fixture-only` need hardware verification before being claimed as read-verified or verified.
- String Matrix for mapped PCM1/PCM2/Modeling string levels, with mute, solo, restore, copy-row-to-all, normalize and percent-scale actions. Unmapped per-string routing/pitch/fine/source-enable fields are shown only in the developer mapping-needed area.
- CTL, EXP, EXP Switch, GK S1/S2, GK Volume and Assign 1-8 registry fields are mapped from a secondary address reference. A representative subset passed USER 73-3 RQ1 read verification; the remaining pedal/assign fields are `fixture-only` until hardware verification.
- Assign Programmer can stage mapped source/target/min/max/mode bytes for confirmed target enum IDs; it does not call these fixture-only bytes verified.
...
- Send imported raw SysEx queue to temporary memory from the advanced utility drawer.
- Save imported mapped parameters only through mapped preview/diff and the normal save/read-back workflow. Unknown raw queues cannot use normal USER save.
...
- Local MCP server with GR-55 status/connect/select/read/write/save/export/import/backup/verify/list/string/assign/control/safety tools.
...
- Full write verification for every individual PCM/modeling/source/effect/pedal/assign control. USER 73-3 read verification passed for many mapped fields; patch name, PCM1 Level, PCM1 String 1 Level, Modeling String 1 Level, Delay Level and EQ Low Gain write/save/read-back passed.
- Full GR-55 assign target/source coverage. A practical subset is mapped for the Assign Programmer; many target IDs and all write behavior still need hardware verification.
...
- That imported third-party patch SysEx queues are safe to save. Unknown raw queues are intentionally blocked from the normal save flow.
